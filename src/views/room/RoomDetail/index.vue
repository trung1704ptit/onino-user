      ...
      <el-row :gutter="15">
        <el-col :xs="24" :sm="24" :lg="5">
          <div class="align-center box box-shadow p-15 mt-15">
            ...
            <p class="white-text m-0" style="font-size: 14px">
              <i>({{ roomDevices.length }} thiết bị đang hoạt động)</i>
            </p>
            ...
            <el-button
              type="danger"
              @click="confirmDelete(roomDetail.id)"
              :loading="deleting"
              class="mt-15 delete-btn"
            >
        ...
        <el-col :xs="24" :sm="24" :lg="19">
          ...
          <div class="box-shadow box notify p-15 mt-15">
            <div class="flex space-between">
              ...
              <span>
                <i class="fa fa-bolt" aria-hidden="true"></i> 269W
              </span>
              <span>
                <i class="fa fa-tint" aria-hidden="true"></i> 100ml
              </span>
            </div>
          </div>
...
export default {
  ...
  mounted() {
    ...
    if (roomList && roomList.length > 0) {
      ...
      if (roomDetail) {
        ...
        new TintColor(roomDetail.groupIconUrl, "#ffffff")
          .run()
          .then(newImage => {
  ...
  methods: {
    ...
    handleUpdateDevice() {
      ...
      this.$store
        .dispatch("device/updateDevice", data)
        .then(response => {
    ...
    getDeviceInfo() {
      this.$refs.deviceForm.validate(valid => {
        if (valid) {
          ...
          this.$store
            .dispatch("device/registerDevice", data)
            .then(response => {
